...
# Streamlit app
def main():
    st.set_page_config(page_title="Waste Management AI", page_icon="♻️", layout="wide")
    ...
    st.markdown(
        ...
        <p style='text-align: center; font-size: 20px;'>
            Upload an image or a video of waste, and let our AI classify it for you!
        </p>
        """,
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            st.markdown(f"<p style='font-size: 24px; font-weight: bold;'>{predicted_category}</p>", unsafe_allow_html=True)
            ...
            with st.expander("Show Disposal Information"):
                st.write(disposal_text)

            st.subheader("Recommended Dustbin")
            ...
            if dustbin_image_path is not None:
                try:
                    ...
                    st.warning(f"Dustbin image not found for {predicted_category}.")
